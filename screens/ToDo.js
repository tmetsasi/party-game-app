...
export default function ToDo({ navigation }) {
  ...
  let showSendVerificationEmail = () => {
    return (
      <View>
        <Text>Vahvistusviestin lähetys:</Text>
        <Button title="Tästä" onPress={() => sendEmailVerification(auth.currentUser)} />
      </View>
    );
  };
  ...
  return (
    <SafeAreaView>
      <View style={[AppStyles.rowContainer, AppStyles.rightAligned, AppStyles.rightMargin, AppStyles.topMargin]}>
        <InlineTextButton text="Käyttäjätilin hallinta" color="#258ea6" onPress={() => navigation.navigate("ManageAccount")}/>
      </View>
      <Modal
        animationType="slide"
        transparent={true}
